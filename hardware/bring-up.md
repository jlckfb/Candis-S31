...
2. Run `i2c_scan lp`, `pmic_test`, and `rtc_test`; compare battery and VBUS readings with a meter. If the RTC reports invalid retained time, set a known value with `rtc_set`, power-cycle the board, and check it again.
...
4. Run `display_test` and inspect all four colors, then record `mark display pass|fail`.
