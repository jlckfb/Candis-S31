# Candis-S31 LVGL compatibility port

This repository-local compatibility component is derived from Espressif's
`esp_lvgl_port` 2.9.0 and retains its public API and Apache-2.0 notices. The
Candis-S31 copy adds the TE synchronization/observer hooks required by the
board's display diagnostics. It is not an ESP-IDF source-tree modification and
is resolved by the local board component manifest.
