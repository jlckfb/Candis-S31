...
static void cdc_rx_callback(int itf, cdcacm_event_t *event)
{
    ...
    if (error == ESP_OK && message.length > 0) {
        xQueueSend(s_cdc_queue, &message, 0);
    }
}
...
static void cdc_line_state_callback(int itf, cdcacm_event_t *event)
{
    ...
    ESP_LOGI(TAG, "Windows DTR=%d RTS=%d", s_dtr,
...
void app_main(void)
{
    ...
    /* Windows is the Source; keep the board in Sink role and OTG 5 V off. */
    ...
    ESP_LOGI(TAG, "USB CDC device ready; waiting for Windows host");
    while (true) {
        cdc_message_t message;
        if (xQueueReceive(s_cdc_queue, &message, pdMS_TO_TICKS(100))) {
            process_command((char *)message.data);
