/*
 * Candis-S31 watch demo - physical input service implementation.
 *
...
static svc_input_cb_t s_callback;
static void *s_user;
...
static void boot_key_poll(void)
{
    ...
    case BOOT_HELD:
        if (!pressed) {
            if (!long_press_fired) {
                input_report(SVC_INPUT_BOOT_SHORT);
            }
            state = BOOT_IDLE;
        } else {
            held_ms += INPUT_POLL_PERIOD_MS;
            if (!long_press_fired && held_ms >= BOOT_LONG_PRESS_MS) {
                ...
                input_report(SVC_INPUT_BOOT_LONG);
            }
        }
        break;
    }
...
static void input_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(INPUT_POLL_PERIOD_MS)) > 0) {
            pwr_key_service();
        }
        boot_key_poll();
    }
...
esp_err_t svc_input_start(svc_input_cb_t cb, void *user)
{
    ...
    if (xTaskCreate(input_task, "svc_input", INPUT_TASK_STACK_BYTES, NULL,
