...
Candis-S31 recovery starts with the ESP32-S31 ROM download mode. It does not depend on a working application, display, touch controller, or filesystem. Run it only with a matching release artifact.
...
6. Verify the SHA-256, then run `flash_factory.sh` from an activated ESP-IDF environment.
...
Activate the ESP-IDF installation that supports ESP32-S31. Recovery requires
esptool 5.3.0 or newer. Check the tool and ROM communication first:
...
The installed package must contain the ESP32-S31 flasher stub. Confirming the
`esp32s31` stub in the esptool package is the minimum environment check.
...
all Factory results persisted in NVS, including any EVT history and the
console-failure counter. Before reflashing a board already under test, save
the latest `host_tools/run_evt.py` JSON and complete console log; after
reflashing, treat every result as `NOT_RUN` and start a new run.
...
image SHA-256 and refuses to emit a TOML that still carries those
placeholders. The TOML uses an explicit `0x0` address for the merged image.
The self-hosted ESP Launchpad build and its bundled esptool-js must recognize
`esp32s31`; verify
