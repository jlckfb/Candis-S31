...
{
    return fusb303b_update_bits(handle, FUSB303B_REG_CONTROL1,
                            ...
                            fusb303b_current_t current)
{
    ...
    ESP_RETURN_ON_ERROR(fusb303b_update_bits(handle, FUSB303B_REG_CONTROL,
                        ...
                        (uint8_t)(current_value << 1)), TAG,
                        "source current update failed");
...
{
    uint8_t events[2] = {0};
    ESP_RETURN_ON_ERROR(fusb303b_read(handle, FUSB303B_REG_INTERRUPT,
    ...
    return fusb303b_write(handle, FUSB303B_REG_INTERRUPT, events,
                          sizeof(events));
}
                              ...
                              bool clear_interrupts)
{
    ...
    const uint8_t current = (status->status &
                             FUSB303B_STATUS_BC_LEVEL_MASK) >> 1;
    status->advertised_current = current >= 3 ? FUSB303B_CURRENT_3_0_A :
                                 current == 2 ? FUSB303B_CURRENT_1_5_A :
